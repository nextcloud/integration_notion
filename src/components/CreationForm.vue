			...
			<div v-for="(field, fieldId) in creationFields"
				...
				<div v-else-if="field.type === 'ncColor'">
					<NcColorPicker
						...
						<NcButton
							v-tooltip.top="{ content: t('integration_notion', 'Choose color') }"
		...
		onOkClick() {
			...
			Object.keys(this.creationFields).forEach((fieldId) => {
				...
				if (field.mandatory && !fieldValue && fieldValue !== false) {
					showError(t('integration_notion', 'Field "{name}" is missing', { name: field.label }))
					isFormValid = false
				}
			})
			if (isFormValid) {
