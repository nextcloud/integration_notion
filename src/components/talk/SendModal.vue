<template>
	<NcModal
		...
		<div class="modal-content">
			<h2>
				{{ t('integration_notion', 'Send link to a Talk room') }}
			</h2>
			<div class="input-wrapper">
				<input ref="query"
					v-model="query"
					type="text"
					:placeholder="t('integration_notion', 'Search for users, groups or conversations')"
			...
			<div class="results">
				<NcEmptyContent v-if="noResults"
					...
					:title="t('integration_notion', 'No conversation found')">
					<template #icon>
						<MagnifyIcon />
					</template>
				</NcEmptyContent>
				<div v-show="conversationsToShow.length > 0" id="conversations">
					<h3>
						{{ t('integration_notion', 'Conversations') }}
					</h3>
					<ul>
	...
	methods: {
		...
		createRoom() {
			...
			axios.post(url, req).then((response) => {
				...
				showSuccess(t('integration_notion', 'You created a conversation with {name}', { name: this.selectedRoom.displayName }))
				this.selectedRoom.token = response.data.ocs.data.token
				this.sendLink()
			}).catch((error) => {
				showError(
					t('integration_notion', 'Failed to join')
		...
		joinOpenRoom() {
			...
			axios.post(url, req).then((response) => {
				...
				showSuccess(t('integration_notion', 'You joined {name}', { name: this.selectedRoom.displayName }))
				this.sendLink()
			}).catch((error) => {
				showError(
					t('integration_notion', 'Failed to join')
		...
		sendLink() {
			...
			axios.post(url, req).then((response) => {
				console.debug('TALK response', response)
				showSuccess(t('integration_notion', 'Link sent to {name}', { name: this.selectedRoom.displayName }))
				this.$emit('close')
			}).catch((error) => {
				showError(
					t('integration_notion', 'Failed to send link')
					+ ': ' + (error.response?.data?.error ?? error.response?.request?.responseText ?? '')
				)
				console.debug(error)
			}).then(() => {
