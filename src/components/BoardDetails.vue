			...
			<div class="links">
				<div class="buttons">
					...
					<a :href="boardLink" target="_blank">
						<NcButton>
							...
							{{ t('integration_notion', 'Open in a new tab') }}
						</NcButton>
					</a>
					<div v-if="talkEnabled"
						...
						<NcButton @click="showTalkModal = true">
							...
							{{ t('integration_notion', 'Share link to a Talk conversation') }}
						</NcButton>
				...
				<div class="link">
					<div class="leftPart">
						...
						<label>
							{{ t('integration_notion', 'Board link') }}
						</label>
					</div>
					<div class="rightPart linkInputWrapper">
						...
						<a :href="boardLink" @click.prevent.stop="copyLink(false)">
							<NcButton v-tooltip.bottom="{ content: t('integration_notion', 'Copy to clipboard') }">
								<template #icon>
				...
				<div v-for="(field, fieldId) in fieldsToDisplay"
					...
					<div class="rightPart">
						<label v-if="['ncCheckbox'].includes(field.type)"
							...
							{{ board[fieldId] ? t('integration_notion', 'Enabled') : t('integration_notion', 'Disabled') }}
						</label>
	...
	methods: {
		async copyLink() {
			...
			try {
				...
				showSuccess(t('integration_notion', 'Board link copied!'))
				...
				console.error(error)
				showError(t('integration_notion', 'Link could not be copied to clipboard'))
			}
		},
